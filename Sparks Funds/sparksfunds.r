...
# But keep an eye on structure as sapply output is matrix
# str(rounds)
...
companies<- mutate_if(companies, is.character, tolower)
str(companies)
rounds<- mutate_if(rounds, is.character, tolower)
str(rounds)
...
# Q.4 Are there any companies in the rounds file which are not present in companies?
sum(!companies$permalink %in% rounds$company_permalink)


# Q.5 Merge the two data frames as master_frame.
#     How many observations are present in master_frame?

master_frame <- merge(rounds, companies, 
                      by.x = 'company_permalink', by.y = 'permalink')
...
type_wise_avg <- group_by(master_frame, funding_round_type)

type_wise_avg <- summarise(type_wise_avg, 
                           avg_funding = mean(raised_amount_usd,
                                              na.rm = TRUE),
                           no_of_rounds = n())

arrange(type_wise_avg, avg_funding)
...
country_wise_gp <- summarise(country_wise_gp, total_rounds = n(),
                             total_funding = sum(raised_amount_usd, na.rm = TRUE))

arrange(country_wise_gp,desc(total_rounds))

arrange(country_wise_gp, desc(total_funding))
...
# which was not meant to be replaced. Correct it.
mapping$category_list <- str_replace_all(mapping$category_list,
                                         "2.na", "2.0")
...
# Check for blank
colSums(new_mapping== "" | new_mapping== " ")
new_mapping[new_mapping$category_list =="", ]
...
rm(mapping, long_mapping, new_mapping, master_frame, sectors)
